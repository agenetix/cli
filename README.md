...
To test a CLI change inside a Agenetix SaaS PR preview, add the ref to `infra/preview-packages.json` in that PR:
...
The Agenetix preview deploy installs that package before image builds, runs a CLI smoke check, and includes the resolved npm package link in the Agenetix PR preview comment.
