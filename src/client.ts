...
export class EmcyHttpError extends Error {
  constructor(
    message: string,
...
export class EmcyClient {
  ...
  static async create(options: GlobalOptions): Promise<EmcyClient> {
    const config = await loadConfig();
    const apiUrl = options.apiUrl
      ?? process.env.AGENETIX_API_URL
      ?? config?.apiUrl
      ?? DEFAULT_API_URL;
    return new EmcyClient(options, apiUrl, config);
  }
